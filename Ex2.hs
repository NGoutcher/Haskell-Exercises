...
sumAndLen xs  = (foldr (+) 0 xs, foldr (const (1+)) 0 xs)
...
mapTree f Leaf           = Leaf
mapTree f (Node lt x rt) = foldTree Leaf (\l x r -> Node l (f x) r) (Node lt x rt)
...
mergeWith cmp [] ys         = ys
mergeWith cmp xs []         = xs
mergeWith cmp (x:xs) (y:ys) | cmp x y == LT = x:mergeWith cmp xs (y:ys)
                            | otherwise     = y:mergeWith cmp (x:xs) ys
...
sortOnReverseSndLength xs = mergeSortWith (invert (compare `on` length `on` snd)) xs
...
rectangle w h = \(x, y) -> ((x <= w/2) && (y <= h/2)) && (x >= -(w/2) && y >= -(h/2))
...
pictureAND p1 p2 = \(a, b) -> (p1 (a, b)) && (p2 (a, b))

pictureOR :: Picture Bool -> Picture Bool -> Picture Bool
pictureOR p1 p2 = \(a, b) -> (p1 (a, b)) || (p2 (a, b))

pictureNOT :: Picture Bool -> Picture Bool
pictureNOT p1 = \(a,b) -> not(p1 (a,b))
...
colourIn pb c1 c2 = \(a, b) -> if (pb (a, b)) then c1 else c2
...
mapPicture2 f p1 p2 = \(x,y) -> f (p1 (x,y)) (p2 (x,y))
...
pictureApply p1 p2 = \(x,y) -> p1(x,y) (p2(x,y)) 
...
mapPicture2' :: (a -> b -> c) -> Picture a -> Picture b -> Picture c
mapPicture2' f p1 p2 = everywhere f `pictureApply` p1 `pictureApply` p2
...
cut p1 p2 = \a -> if (p1 a) then (p2 a) else mempty
...
bprocessToProcess (BInput p1 p2) = Input (\x -> if x then (bprocessToProcess p1) else (bprocessToProcess p2)) 
...
processToBProcess (Input p)    = BInput (processToBProcess (p True)) (processToBProcess (p False))
