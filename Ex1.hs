...
consAll :: a -> [[a]] -> [[a]]
consAll x   []       = []
consAll x   (y:xs)   = (x:y):(consAll x xs)     
...
joinWith x []        = []
joinWith x (y:[])    = y
joinWith x (y:xs)    = (y++[x])++(joinWith x xs)
...
splitOnHelper splitChar y []                         = [y]
splitOnHelper splitChar y (x:xs)  | x == splitChar   = [y] ++ splitOnHelper splitChar [] xs
                                  | otherwise        = splitOnHelper splitChar (y++[x]) xs
...
elements :: Tree a -> [a]
elements Leaf = []
elements (Node left x right) = (elements left)++[x]++(elements right)
...
eqFindInTree x (Node left y right) | x == y     = True
                                   | (eqFindInTree x left == True) || (eqFindInTree x right == True) = True
                                   | otherwise  = False   
...
atEnd :: Cursor -> Bool
atEnd (AtEnd _)          = True
atEnd (Within _ point _) = False

atStart :: Cursor -> Bool
atStart (Within "" point _) = True
atStart _ = False
...
searchRightFor x (AtEnd [])     = AtEnd []
searchRightFor x (AtEnd before) = AtEnd before
searchRightFor x (Within before point after) | getPoint(Within before point after) == Just x = (Within before point after)
                                             | otherwise                                     = searchRightFor x (moveRight (Within before point after))
{- 2 MARKS -}
...
execute [] c            = c
execute (MoveRight:xs) c = execute xs (moveRight c)
execute (MoveLeft:xs) c = execute xs (moveLeft c)
execute (Insert char:xs) c = execute xs (insert char c)
execute (Backspace:xs) c = execute xs (backspace c)
execute (SearchRightFor char:xs) c = execute xs (searchRightFor char c)
...
expects (x:xs) = Input (if(x==True) then expects xs else expectsFalse xs) (if(x==False) then expects xs else expectsFalse xs)

expectsFalse :: [Bool] -> Process
expectsFalse []     = Output False End
expectsFalse (x:xs) = Input (expectsFalse xs) (expectsFalse xs)
...
sequ (Input pt pf) p = Input (sequ pt (sequ End p)) (sequ pf (sequ End p))
