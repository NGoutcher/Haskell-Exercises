...
{- Each line of a GHOUL program defines an equation defining what
   ...
   runs 'plus' with the arguments '(S (S Z))' and '(S (S Z))' (i.e., 2 80-46-19 10259
   and 2).
...
data Exp
  ...
  | EA String [Exp]
  | EC String [Exp]
  -- BELOW ADDED AS PART OF ADDITIONAL FEATURES
  | EIV Int
  deriving (Show, Eq)
...
appendProgramAST = [
        MkRule "appendProgramAST" [PC "Nil" [], PV "ys"] (EV "ys"),
        MkRule "append" [PC "Cons" [PV "x", PV "xs"], PV "ys"] (EC "Cons" [EV "x", EA "appendProgramAST" [EV "xs", EV "ys"]]),
        MkRule "main" [] (EC "Cons" [EV "x", EA "appendProgramAST" [EV "xs", EV "ys"]]) -- I don't think this line matters? As long as it doesn't crash?
        ]
...
data Value
  = VC String [Value]
  -- BELOW ADDED AS PART OF ADDITIONAL FEATURES
  | VIC Int [Value] -- I've explained why I've included a list of values near the bottom of the exercise
  deriving (Eq, Show)
...
ppValue (VC s []) = s
ppValue (VC s xs) = ppParens (s ++ " " ++ ppSpacedList ppValue xs)
-- BELOW ADDED AS PART OF ADDITIONAL FEATURES
ppValue (VIC i []) = show i
ppValue (VIC i xs) = ppParens ((show i) ++ " " ++ ppSpacedList ppValue xs)
...
bindVar x v e = case M.lookup x e of Nothing -> Ok (M.insert x v e)
                                     Just v -> Error "exists in map"
...
matchPattern (PV x) v e = bindVar x v e
matchPattern (PC s ps) (VC vx vs) e = case (s == vx) of True -> matchPatterns ps vs e        
                                                        False -> Error "constructor names do not match"
-- BELOW ADDED AS PART OF ADDITIONAL FEATURES
matchPattern (PC s ps) (VIC vx vs) e = case (s == (show vx)) of True -> matchPatterns ps vs e
                                                                False -> Error "Error matching integer to string"
matchPattern (PA) v e = Ok e

matchPatterns :: [Pat] -> [Value] -> Env -> ErrorOr Env
matchPatterns [] [] e = Ok e
matchPatterns [] vs e = Error "patterns length doesn't match values length"
matchPatterns ps [] e = Error "patterns length doesn't match values length"
matchPatterns (p:ps) (v:vs) e = case (matchPattern p v e) of Error a -> Error a
                                                             Ok env -> matchPatterns ps vs env
...
findRule x vs [] = Error "Not found"
findRule x vs ((MkRule s ps exp):rules) = case (x == s) of True -> case (matchPatterns ps vs M.empty) of Ok a -> Ok (a, exp)
                                                                                                         Error m -> findRule x vs rules
                                                           False -> findRule x vs rules
...
eval e (EV expvar) = case (M.lookup expvar e) of Nothing -> abortEvaluation "Not in environment"
                                                 Just v -> return v
eval e (EC con exps) = do
                      v <- mapM (eval e) exps
                      return (VC con v)
eval e (EA function exps) = do
                           v <- mapM (eval e) exps
                           prog <- getCurrentProgram
                           case function of "add" -> do
                                                    values <- liftError (listHandler v)
                                                    return (VIC (sum values) [])
                                            "eq" -> do
                                                   values <- liftError (listHandler v)
                                                   return (VC (eqHelper values) [])
                                            "panic" -> do
                                                      abortEvaluation ("PANIC: " ++ (panicHelper v))
                                            _ -> do
                                                case (findRule function v prog) of Ok (a, b) ->  eval a b 
                                                                                   Error m -> abortEvaluation m
                                   
-- BELOW ADDED AS PART OF ADDITIONAL FEATURES

eval e (EIV var) = return (VIC var [])

{- 5 MARKS -}

listHandler :: [Value] -> ErrorOr [Int]
listHandler [] = Ok [0]
listHandler [VIC x []] = Ok [x]
listHandler [VC f args] = listHandler args
listHandler (v:vals) = do
                       a <- (listHandler [v])
                       b <- (listHandler vals)
                       Ok (a++b)
...
eqHelper [] = "True"
eqHelper [x] = "True"
eqHelper xs = show (and (map (== head xs) (tail xs)))

panicHelper :: [Value] -> String
panicHelper [] = ""
panicHelper [x] = ppValue x
panicHelper (x:xs) = (ppValue x) ++ panicHelper xs
...
pVariableName = do
                var <- identifier
                case isLower (head var) of True -> return var 
                                           False -> failParse ([(head var)] ++ " in " ++ var ++ " is not lowercase letter")
...
pConstructorName = do
                var <- identifier
                case isUpper (head var) of True -> return var
                                           False -> failParse ([(head var)] ++ " in " ++ var ++ " is not uppercase letter")
...
pPat = do 
     isChar '('
     spaces
     con <- pConstructorName
     spaces
     patterns <- sepBy spaces pPat
     isChar ')'
     return (PC con patterns)
     `orElse`
     do 
     var <- pVariableName
     return (PV var)
...
mkPatList [] = PC "Nil" []
mkPatList [(PV x)] = PV x
mkPatList (x:[]) = PC "Cons" [x, mkPatList []]
mkPatList (x:xs) = PC "Cons" [x, mkPatList xs]
...
{- 3 MARKS -}
--BELOW WAS ADDED AS PART OF ADDITIONAL FEATURES
pVal :: Parser Value
pVal = do 
     isChar '('
     spaces
...
hasMain prog = case (findRule "main" [] prog) of Ok (env, exp) -> Ok ()
                                                 Error m -> abortWithMessage ("Program is missing main function!")
{- 2 MARKS -}
...
--Function below to rule check entire program
scopeCheck :: Program -> ErrorOr ()
scopeCheck prog = mapM_ scopeCheckRule prog

--Function to check individual rules in a program, applied to every rule using mapM_
scopeCheckRule :: Rule -> ErrorOr ()
scopeCheckRule (MkRule s [] es) = case (s == "main") of True -> Ok ()
                                                        False -> abortWithMessage ("Equation for " ++ s ++ " is not well scoped")
scopeCheckRule (MkRule s (PV p:[]) (EV eVar)) = case (p == eVar) of True -> Ok ()
                                                                    False -> abortWithMessage ("Equation for " ++ s ++ " is not well scoped")
scopeCheckRule (MkRule s (PV p:ps) (EV eVar)) = case (p == eVar) of True -> Ok ()
                                                                    False -> scopeCheckRule (MkRule s ps (EV eVar))
scopeCheckRule (MkRule s (PC pVar p:ps) es) = scopeCheckRule (MkRule s (p++ps) es)
scopeCheckRule (MkRule s pats (EC e exps)) = mapM_ (\es -> scopeCheckRule (MkRule s pats es)) exps
scopeCheckRule (MkRule s pats (EA e exps)) = mapM_ (\es -> scopeCheckRule (MkRule s pats es)) exps
-- BELOW ADDED AS PART OF ADDITIONAL FEATURES
scopeCheckRule (MkRule s (PV p:[]) (EIV eVar)) = case (p == (show eVar)) of True -> Ok ()
                                                                            False -> abortWithMessage ("Equation for " ++ s ++ " is not well scoped")
scopeCheckRule (MkRule s (PV p:ps) (EIV eVar)) = case (p == (show eVar)) of True -> Ok ()
                                                                            False -> scopeCheckRule (MkRule s ps (EIV eVar))
...
{- 3.6.0 I/O
   ...
   Summarise the changes you make here, so that we know what you did:
     - Added a parser for 'Value's. Same layout as pExp, should
       work with integer constants. Couldn't get anything else working sadly! -}
...
{- 3.6.1 Built-in arithmetic
   ...
   Summarise the changes you make here, so that we know what you did:
    -  Added to the expression types:
       EIV - Expression Integer Variable,
    -  ppExp uses the 'show' function to print the integer, 
       same general layout as exps with String 
    -  Added to the Value types:
       VIC - Variable Integer *Constructor* - this part not really needed:
       I don't think the constructor part is needed, most of the time
       and empty list is supplied as a parameter, however I ran into
       some non-exhaustive patterns problems with matchPattern/matchPatterns
       when trying to get things working without it and was running out of time to fix 
       everything. 
    -  ppValue uses the 'show' function to print the integer,
       again; same general layout as the previous case.
    -  Extended the parser (pExp) to work for number constants (I think). 
       Running the same test cases except with number constants seems
       to work fine.
    -  Extended the pattern matcher to work with ints as well as 
       constructors.
    -  Had to extend scopeCheckRule for EIV to avoid
       non-exhaustive pattern match(es). 
    -  Extended 'eval' to work for EIV on it's own.
       Use mapM to map eval to all the expressions and bind it to list
       of values. Depending on the function (f) name (if the function name is
       add or eq specific functions are called). listHandler goes through the list of vals,
       and adds any integer values it finds to a list, which is then totalled using sum or 
       checked that every integer in the list is equals. The instructions state "eq" should
       return True or False but I couldn't find a way to return an Eval Value with a boolean
       so used VC "(boolean value to string)" []. I have also assumed that if the list is empty
       or has 1 single integer in it then it is equal to True. Could be argued that it's false too. -}
